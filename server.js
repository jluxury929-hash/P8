/**
 * ===============================================================================
 * APEX MASTER v35.1 (PINNACLE QUANTUM SINGULARITY) - FINAL REPAIR BUILD
 * ===============================================================================
 * DNA: WHALE HUNTER + DYNAMIC SCALING + HEALTH SERVER + NUCLEAR BRIBE
...
if (cluster.isPrimary) {
    ...
    console.log(`║   ⚡ APEX MASTER v35.1 | PINNACLE QUANTUM SINGULARITY║`);
    console.log(`║   DNA: WHALE HUNTER + WEALTH-SCALING + RESILIENCE   ║`);
    ...
    for (let i = 0; i < cpuCount; i++) {
        ...
        }, i * 1500); // 1.5s Staggered launch to prevent provider rate-limits
    }
} else {
    // --- WORKER CORE ---
    ...
    try {
        http.createServer((req, res) => {
            if (req.url === '/status') {
                ...
                res.end(JSON.stringify({ status: "ACTIVE", role: ROLE, chain: CHAIN.name }));
            }
    ...
    async function connect() {
        try {
            ...
            if (ROLE === "LISTENER") {
                ...
                ws.on("pending", async (txH) => {
                    ...
                    if (valueEth >= parseFloat(formatEther(GLOBAL_CONFIG.MIN_WHALE_VALUE))) {
                        if (valueEth >= GLOBAL_CONFIG.WHALE_HUNTER_THRESHOLD) {
                            console.log(`\n${TAG} ${TXT.red}${TXT.bold}🚨 PINNACLE WHALE: ${valueEth.toFixed(2)} ETH detected${TXT.reset}`);
                        }
                ...
                console.log(`${TAG} Pinnacle Peering Engaged.`);
            } else if (ROLE === "STRIKER") {
                process.on('message', async (msg) => {
                    if (msg.type === 'SIGNAL' && msg.chainId === CHAIN.chainId) {
                        await executePinnacleStrike(provider, wallet, poolIface, l1Oracle, currentEthPrice, CHAIN, TAG);
                    }
                });
            }
...
async function executePinnacleStrike(provider, wallet, poolIface, l1Oracle, ethPrice, CHAIN, TAG) {
    try {
        ...
        else if (usdValue >= 100) loanAmount = parseEther("50");
        ...
        if (netProfit > parseEther(GLOBAL_CONFIG.MIN_NET_PROFIT)) {
            console.log(`\n${TXT.green}${TXT.bold}💎 PINNACLE STRIKE: +${formatEther(netProfit)} ETH | Leverage: ${formatEther(loanAmount)} ETH${TXT.reset}`);
