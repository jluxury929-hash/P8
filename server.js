// ===============================================================================
// APEX ULTIMATE TITAN v54.0 (ELITE STABILITY MERGE) - HIGH-FREQUENCY CLUSTER
// ===============================================================================
...
const GLOBAL_CONFIG = {
    ...
    MEMPOOL_SAMPLE_RATE: 0.012,          // 1.2% per core (Full coverage across 48 cores)
    WORKER_BOOT_DELAY_MS: 20000,         // 20s Master Stagger (RPC handshake safety)
    HEARTBEAT_INTERVAL_MS: 45000,        
    RPC_COOLDOWN_MS: 15000,              
    RATE_LIMIT_SLEEP_MS: 180000,         // 3m Deep Sleep if 429 persists
    
    // 🐋 DYNAMIC STRATEGY SETTINGS (v32.0 Merge)
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v54.0 | ELITE STABILITY MASTER       ║
║   MERGE: v32.0 DYNAMIC + 48-CORE HANDSHAKE BYPASS    ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);

    const cpuCount = Math.min(os.cpus().length, 48); 
    console.log(`${TXT.cyan}[SYSTEM] Initializing 48-Core Fleet with 20s Safe-Stagger...${TXT.reset}`);
...
// --- WORKER PROCESS ---
    ...
    // Tier 2 Stagger: Workers wait 5-25s before even attempting connection
...
async function initWorker(CHAIN) {
    ...
    async function safeConnect() {
        ...
        try {
            // v54.0 FIX: Hardcode Network to bypass all Infura/Alchemy Chain Probes
            ...
            // Load-Balanced Heartbeat (Syncing fees/prices on long intervals)
            setInterval(async () => {
            ...
            }, GLOBAL_CONFIG.HEARTBEAT_INTERVAL_MS + (Math.random() * 10000));
            ...
            // v54.0 FIX: Tier 3 Delay (15s) before Mempool/Log subscriptions allowed
            setTimeout(() => {
                // MEMPOOL INTERCEPTOR
                wsProvider.on("pending", async (txHash) => {
                    ...
                    try {
                        ...
                        if (tx && tx.to && tx.value >= GLOBAL_CONFIG.WHALE_THRESHOLD) {
                            ...
                            if (isDEXTrade) {
                                console.log(`\n${TAG} ${TXT.gold}⚡ OPPORTUNITY DETECTED: ${formatEther(tx.value)} ETH whale transaction!${TXT.reset}`);
...
async function attemptStrike(provider, wallet, iface, gasOracle, pool, ethPrice, CHAIN, mode, feeData) {
    try {
        ...
        // v32.0 DUAL SCALING (Wealth Tiers + Pool Reserve Check)
